...
// ─── Legacy-compatible exports (merged from autoDoc.ts) ───

export type LegacyDocEntry = {
  id: string;
  agent: string;
  taskId: string;
  feature: string;
...
/**
 * @deprecated Use generateTaskDocumentation instead
 * Legacy wrapper for backward compatibility with autoDoc.ts consumers
 */
export async function autoGenerateDocs(input: {
  ...
  const result = await generateTaskDocumentation({
    repoPath: input.repoPath,
    taskId: input.taskId,
    taskTitle: input.feature,
    agent: input.agent,
    filesModified: [],
    summary: input.description,
    commitMode: input.commitMode,
  });
  return { docId: `doc-${input.taskId}-${Date.now()}`, docPath: result.docPath };
}

/**
 * @deprecated Use listDocumentation instead
 * Legacy wrapper for backward compatibility with autoDoc.ts consumers
 */
export async function listDocs(repoPath?: string): Promise<LegacyDocEntry[]> {
  const result = await listDocumentation(repoPath);
  return result.docs.map(d => ({
    ...
    filesChanged: d.filesModified,
    createdAt: new Date(d.createdAt).getTime() || Date.now(),
  }));
}
